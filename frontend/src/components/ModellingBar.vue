...
import { X, RotateCcw, Save, Copy } from 'lucide-vue-next'
...
const saveFork = async () => {
    try {
        ...
        // Reset local modelling and switch to new scenario
...
<template>
  <aside class="w-80 bg-white border-l border-slate-200 flex flex-col z-10 flex-shrink-0 shadow-xl h-full">
    <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex-shrink-0">
        <h2 :class="['text-sm font-bold uppercase tracking-wider mb-4 transition-colors', isModelling ? 'text-[#635bff]' : 'text-slate-900']">
            Modelling
        </h2>
