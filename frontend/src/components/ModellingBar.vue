...
// Helper to check if item is date type
const isDateType = (item) => item.inputType === 'date' || (item.field && item.field.includes('date'));
const isBooleanType = (item) => item.inputType === 'boolean' || typeof item.value === 'boolean';

const toggleBoolean = (item) => {
    store.updateOverride(item.id, !store.overrides[item.id]);
}

const formatOrig = (item) => {
    if (isBooleanType(item)) return item.value ? 'Enabled' : 'Disabled';
    return item.value;
}
</script>
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div v-else v-for="item in store.pinnedItems" :key="item.id" class="group">
                ...
                <div v-else-if="isDateType(item)">
                    <input type="date" :value="store.overrides[item.id]" @change="e => updateValue(item, e)" class="w-full border border-slate-300 rounded px-2 py-1.5 text-xs focus:border-primary">
                </div>
                <div v-else-if="isBooleanType(item)">
                    <div class="flex items-center justify-between h-8 px-1">
                        <span class="text-xs text-slate-500">{{ store.overrides[item.id] ? 'Enabled' : 'Disabled' }}</span>
                        <button @click="toggleBoolean(item)" 
                            :class="['relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2', store.overrides[item.id] ? 'bg-[#635bff]' : 'bg-slate-200']">
                            <span :class="['pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out', store.overrides[item.id] ? 'translate-x-4' : 'translate-x-0']"></span>
                        </button>
                    </div>
                </div>
                <div v-else>
                    ...
                    <div class="flex justify-between items-center mt-1 text-[10px] text-slate-400 font-mono"><span>Orig: {{ formatOrig(item) }}</span></div>
                </div>
            </div>
        </div>
