...
export const useSimulationStore = defineStore('simulation', () => {
    ...
    async function saveEntity(type, id, data, description = "Update") {
        ...
        try {
            ...
            // Fix 422 for Income Fields
            if (payload.salary_sacrifice_value !== undefined) payload.salary_sacrifice_value = Math.round(payload.salary_sacrifice_value * 100);
            if (payload.taxable_benefit_value !== undefined) payload.taxable_benefit_value = Math.round(payload.taxable_benefit_value * 100);
            if (payload.employer_pension_contribution !== undefined) payload.employer_pension_contribution = Math.round(payload.employer_pension_contribution * 100);
            ...
            // Clean Dates
            if ('birth_date' in payload && payload.birth_date === "") payload.birth_date = null;
            ...
            if (type === 'account') {
                ...
                if (id === 'new') await api.createStrategy(activeScenarioId.value, payload);
                else await api.updateStrategy(activeScenarioId.value, id, payload);
            }
    ...
    async function deleteEntity(type, id) {
        ...
        try {
            ...
            else if (type === 'strategy') url = `/api/strategies/${id}`;
