    ...
    <div class="flex flex-col min-h-full pb-24 max-w-5xl mx-auto w-full relative pb-24">
        ...
        <div v-else class="space-y-8">
            ...
            <div v-if="accountsByType.rsu && accountsByType.rsu.length > 0">
                <h3 class="text-sm font-bold text-indigo-600 uppercase tracking-wider mb-3 px-1">Unvested Equity</h3>
                <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                    <table class="w-full text-left text-sm">
                        <thead class="bg-slate-50 border-b border-slate-200 text-slate-500">
                            <tr><th class="px-6 py-3 font-medium">Grant Name</th><th class="px-6 py-3 font-medium text-right">Units</th><th class="px-6 py-3 font-medium text-right">Price</th><th class="px-6 py-3 font-medium text-center w-16"></th></tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            <tr v-for="acc in accountsByType.rsu" :key="acc.id" class="group hover:bg-slate-50/50 transition-colors">
                                <td class="px-6 py-4"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center"><Briefcase class="w-4 h-4" /></div><div class="font-medium text-slate-900">{{ acc.name }}</div></div></td>
                                <td class="px-6 py-4 text-right font-bold text-slate-700">{{ acc.starting_balance / 100 }}</td>
                                <td class="px-6 py-4 text-right font-mono text-slate-600">{{ formatPounds(acc.unit_price) }}</td>
                                <td class="px-6 py-4 text-center"><button @click="openEdit(acc)" class="p-1.5 text-slate-300 hover:text-primary hover:bg-slate-100 rounded-md transition-all"><Pencil class="w-4 h-4" /></button></td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <Drawer :isOpen="!!editingAccount" :title="editingAccount?.name || 'New Account'" @close="editingAccount = null" @save="save">
            <div v-if="editingAccount" class="space-y-6">
                ...
                <div v-if="form.account_type === 'Mortgage'" class="space-y-6 pt-2">
                    ...
                    <div class="p-4 bg-amber-50 rounded-lg border border-amber-100 space-y-4"><h4 class="text-xs font-bold text-amber-600 uppercase tracking-wide">Early Repayment</h4>
                        <div><div class="flex justify-between items-center mb-1"><label class="block text-sm font-medium text-slate-700">Annual Overpayment Allowance (%)</label><PinToggle v-if="editingAccount.id !== 'new'" :item="{ id: `acc-${editingAccount.id}-overpay`, realId: editingAccount.id, type: 'account', field: 'overpayment_allowance_percent', label: `${editingAccount.name} Overpay Allowance`, value: editingAccount.overpayment_allowance_percent, format: 'percent' }" /></div><input type="number" v-model="form.overpayment_allowance_percent" placeholder="e.g. 10" class="w-full border border-slate-300 rounded-md px-3 py-2 text-sm"><p class="text-xs text-amber-700 mt-1">The lender's permitted annual overpayment as a % of the outstanding balance. Use an <span class="font-semibold">Automation → Mortgage Overpay</span> rule to act on this allowance.</p></div>
                    </div>
                ...
                <div v-else-if="form.account_type === 'RSU Grant'" class="space-y-6 pt-2">
