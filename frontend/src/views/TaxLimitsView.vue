...
import { Scale, Users, Pencil, Check } from 'lucide-vue-next'
...
const openEdit = (item, type) => {
    ...
    if (type === 'tax_limit') {
        form.value = { 
            ...
            // Ensure Arrays are initialized to avoid "Select All" bug
...
const save = async () => {
    ...
    await store.saveEntity(type, editingItem.value.id, form.value)
...
<template>
    <div class="flex flex-col h-full max-w-5xl mx-auto w-full pb-24">
        ...
        <div v-else class="space-y-10">
            <div><h3 class="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3 px-1">Household</h3>
            ...
            <div><h3 class="text-sm font-bold text-indigo-600 uppercase tracking-wider mb-3 px-1">Fiscal Limits (Tax Year)</h3>
        ...
        <Drawer :isOpen="!!editingItem" :title="editingItem?.type === 'owner' ? 'Edit Person' : 'Edit Tax Limit'" @close="editingItem = null" @save="save">
            <div v-if="editingItem" class="space-y-4">
                ...
                <div v-else>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-1"><label class="block text-sm font-medium text-slate-700">Annual Limit</label><PinToggle :item="{ id: `tax-${editingItem.id}`, realId: editingItem.id, type: 'tax_limit', field: 'amount', label: editingItem.name, value: editingItem.amount / 100, format: 'currency' }" /></div>
