...
logger = logging.getLogger("engine.decumulation")
...
def process_decumulation(scenario: models.Scenario, context: ProjectionContext):
    ...
    logger.info(f"[{context.month_start}] Deficit Detected: {utils.format_currency(total_deficit)}. engaging strategy '{active_strategy.name}'")
    ...
    for deficit_record in deficit_accounts:
        ...
        for source in candidates:
            ...
            msg = f"Auto-sold {utils.format_currency(take)} from {source_acc.name} to cover deficit."
            logger.info(f"[{context.month_start}] {msg}")
