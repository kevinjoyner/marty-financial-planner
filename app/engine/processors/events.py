        ...
        if (event.event_date >= context.month_start and event.event_date < next_month_start and event.event_date >= scenario.start_date):
            ...
            if event.event_type == enums.FinancialEventType.INCOME_EXPENSE and event.from_account_id in context.account_balances:
                if event.value > 0:
                     ...
                     track_contribution(context, event.from_account_id, event.value)
                ...
                if from_acc:
                    cost_portion, gain = calculate_disposal_impact(event.value, context.account_balances[event.from_account_id], context.account_book_costs[event.from_account_id], from_acc.account_type, from_acc.tax_wrapper)
